...
export function seedDevelopment(db, { rootDir, env = process.env, writeCredentials = true } = {}) {
  ...
  if(!db.prepare('SELECT 1 FROM beto_activities WHERE account_id=?').get(account.id)){
    recordBetoActivity(db,{accountId:account.id,clientId:client.id,type:'professional_request',title:'Você pediu um orçamento ao BETO',description:'Pintura interna para Gabriel, com medidas e valor informados.',status:'completed'});
